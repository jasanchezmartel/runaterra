...
function SelectedFaction() {
  ...
  const revertPreviousSelection = () => {
    const allRegionElements = document.querySelectorAll(
      '[class*="region-extended region-"]'
    );
  ...
  const SelectedRegionDisplay = ({ regionName }) => {
    ...
    const regionClass = `selected-region-display region-${regionName
      .replace(/\s+/g, "-")
      .toLowerCase()}`;
  ...
  return (
    <div className="selected-faction-container" ref={containerRef}>
      <div className="selection-controls">
        ...
        >
          {isSelecting ? "Seleccionando..." : "Ocultar 2 Regiones Aleatorias"}
        </button>
      </div>
        ...
        <div className="selected-regions-display">
          <div className="selected-regions-grid">
            {selectedRegions.map((region) => (
              <SelectedRegionDisplay key={region} regionName={region} />
            ))}
          </div>
        </div>
      )}
